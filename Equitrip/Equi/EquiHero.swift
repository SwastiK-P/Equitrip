...
struct EquiHeroScene: View {
    var appeared: Bool
    /// Set on iPad to draw the whole scene at that width, scaled from the
    /// phone's proportions and faded out at both sides — at that size it no
    /// longer runs off the window, and a hard vertical edge on a painting reads
    /// as a crop. Nil is the phone: full width, 240pt tall, unfaded.
    ...
    /// Width over height of the box on the phone the coordinates below were
    /// placed against. Scaling at this ratio is what keeps them valid.
    static let phoneAspect: CGFloat = 393 / 240
    ...
    var body: some View {
        ...
        .frame(width: width, height: width.map { $0 / Self.phoneAspect } ?? Self.height)
    ...
    private func landscape(width: CGFloat) -> some View {
        ...
        return Image("mountains")
            ...
            .mask {
                LinearGradient(
                    stops: [
                        .init(color: self.width == nil ? .white : .clear, location: 0),
                        .init(color: .white, location: 0.14),
                        .init(color: .white, location: 0.86),
                        .init(color: self.width == nil ? .white : .clear, location: 1)
                    ],
                    startPoint: .leading,
                    endPoint: .trailing
                )
    ...
    /// How much bigger than the phone's box this one is. The notes grow with
    /// it — gently for the bubble, whose text is still interface — or a 17pt
    /// scrawl beside a painting twice the size shrinks into a footnote.
    private var scale: CGFloat {
        guard let width else { return 1 }
        return max(1, min(width / 393, 1.6))
    }

    private func note(_ text: String, size: CGSize, at unit: CGPoint, tilt: Double) -> some View {
        Text(text)
            .font(EquiHand.font(17 * scale))
    ...
    private func bubble(size: CGSize) -> some View {
        Text("Your next adventure,\nsimpler with Equi.")
            .font(.system(size: 12.5 * min(scale, 1.3), weight: .medium))
